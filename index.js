  ...
  locales.map((local) => {
    if (local.id === event.target.id){
      ...
      document.getElementById('botonesContenedor').innerHTML = `
        <button title="Últimos 11 informes" class="col btn-info rounded px-2 py-1" onclick="formulario1(${local.id})">Últimos reportes</button>
        <div class="col btnMargin btnInput d-flex flex-column align-items-center">
